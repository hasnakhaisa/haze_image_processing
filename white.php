...
//        if ($quan[$b] <= $_GET['id'] && $quan[$b] >= ($_GET['id'] - 10)) {
        echo '<div class="gallery"
            ...
            <div class="desc">nama : ' . $name[$b] . ' level : ' . $quan[$b] . '<p id="haze-' . $name[$b] . '" ><p><p id="stddev-' . $name[$b] . '" ><p><p id="mean-' . $name[$b] . '" ><p></div>
            </div>';
        echo '<script>
            var pix = [];
            var pix1 = [];
			...
			imageObj.src = dir;
				...
				window.canny = CannyJS.canny(myCanvas);
                canny.drawOn(myCanvas);
            
            var canny_context = myCanvas.getContext(\'2d\');

            var dataimg = canny_context.getImageData(x, y, imageObj.width, imageObj.height);
/*				
				var grayscale = Filters.filterImage(Filters.grayscale, imageObj);
                ...
                for (i=0; i<dataimg.data.length; i+=4){
                    ...
                    tempsum += dataimg.data[i];
                    sum++;
                }
                mean = tempsum / sum;
                
                for (i = 0; i < dataimg.data.length; i += 4) {
                    std_tempsum += (mean - dataimg.data[i]) * (mean - dataimg.data[i]);
                }
                ...
                $.ajax({
                    type: \'POST\',
                    url: \'update.php\',
                    data: {haze:haze, name:name, quan:quan, mean:mean, stddev:stddev},
